...
export const ChatMessageInput = ({ disabled, onSend }:Props) => {
    ...
    return(
        <div 
        className={
            `flex border border-gray-800/50 bg-gpt-lightgray p-2 rounded-md ${disabled && 'opacity-50'}`
        }>
            <textarea 
                ref={textEl}
            ...
            <div onClick={handleSend} className={`self-end p-1 cursor-pointer rounded ${text.length ? 'opacity-100 hover:bg-black/20' : 'opacity-20'}`}>
